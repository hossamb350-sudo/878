...
async function startServer() {
  ...
  if (!isProduction) {
    ...
    // SEO Interceptor for news, articles, watch, leader
    app.get(["/news/:id", "/articles/:id", "/watch/:id", "/leader/:id"], async (req, res) => {
      ...
      let title = "منصة تعز الإعلامية";
      let description = "منصة إعلامية يمنية مستقلة تهتم بنقل الأخبار والمستجدات وتغطية الفعاليات والأنشطة المتنوعة.";
      let imageUrl = "https://taiz-media-ye.vercel.app/Resources/app-icon.png";
        ...
        try {
          if (originalPath.startsWith("/news/")) {
            ...
            if (docSnap.exists) {
              ...
              description = data.shortDescription || data.content?.substring(0, 150) || description;
              imageUrl = data.imageUrl || imageUrl;
            }
          } else if (originalPath.startsWith("/articles/")) {
            const docRef = firestore.collection("articles").doc(id);
            const docSnap = await docRef.get();
            if (docSnap.exists) {
              const data = docSnap.data();
              title = data.title || title;
              description = data.content?.substring(0, 150) || description;
              imageUrl = data.authorPhoto || data.imageUrl || imageUrl;
            }
          } else if (originalPath.startsWith("/watch/")) {
            const docRef = firestore.collection("videos").doc(id);
            const docSnap = await docRef.get();
            if (docSnap.exists) {
              const data = docSnap.data();
              title = data.title || title;
          ...
          } else if (originalPath.startsWith("/leader/")) {
            const docRef = firestore.collection("content?").doc(id);
            const docSnap = await docRef.get();
            if (docSnap.exists) {
              const data = docSnap.data();
              title = data.title || title;
              description = data.description || data.content?.substring(0, 150) || description;
              imageUrl = data.thumbnailUrl || imageUrl;
            }
          }
        } catch (e) {
          console.error("SEO dynamic meta fetch error:", e);
        }
      }
      ...
      if (fs.existsSync(indexHtmlPath)) {
        try {
          ...
          const metaTags = `
  <title>${title} | منصة تعز الإعلامية</title>
  <meta name="description" content="${description.replace(/"/g, '&quot;')}" />
  <meta property="og:title" content="${title.replace(/"/g, '&quot;')}" />
  <meta property="og:description" content="${description.replace(/"/g, '&quot;')}" />
  <meta property="og:image" content="${imageUrl}" />
  <meta property="og:url" content="https://taiz-media-ye.vercel.app${originalPath}" />
...
`;
          ...
          console.error("SEO index.html replace error:", htmlErr);
        }
      }
