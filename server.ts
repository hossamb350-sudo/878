...
// Endpoint to detect stream Content-Type dynamically from the server response
app.get("/api/stream/detect-type", async (req, res) => {
  ...
  try {
    ...
    // Attempt HEAD request first (fastest)
    let response;
    try {
      ...
      // Fallback to a lightweight GET request if HEAD is not supported by the stream server
      response = await axios({
        method: "get",
        url: targetUrl,
        timeout: 4000,
      ...
      // Immediately destroy the stream to avoid hanging connections
    ...
    const isAudio = contentType.toLowerCase().startsWith("audio/") || 
                    ...
                    contentType.toLowerCase().includes("aac") || 
                    contentType.toLowerCase().includes("stream");
    ...
    console.warn("Stream detection failed:", error.message || error);
    // Return a fallback based on typical radio keywords if the remote server is offline
    const isLikelyAudio = targetUrl.includes("proxy") || 
                          targetUrl.includes("stream") || 
                          targetUrl.includes("radio") || 
                          targetUrl.includes("168.119.10.136");
    return res.json({ 
      success: false, 
      contentType: "audio/mpeg", 
      isAudio: isLikelyAudio,
      error: error.message 
    });
  }
});
