...
const componentCode = `function AdminUrgentNews() {
  ...
  const [staticMinutes, setStaticMinutes] = useState(60);
  const [scrollingMinutes, setScrollingMinutes] = useState(1440);
  ...
  const saveNews = async () => {
    ...
    setSaving(true);
    try {
      const newStaticExpiresAt = staticMinutes > 0 ? Date.now() + staticMinutes * 60000 : 0;
      const newScrollingExpiresAt = Date.now() + (staticMinutes + scrollingMinutes) * 60000;

      if (staticMinutes > 0) {
        // Enforce only one static news
        const currentStatics = urgentItems.filter(i => i.isActive !== false && i.id !== editingId);
        for (const item of currentStatics) {
          const currentScroll = item.scrollingExpiresAt || item.expiresAt || (Date.now() + 1440 * 60000);
          await updateDoc(doc(db, "urgentNews", item.id), { staticExpiresAt: 0, scrollingExpiresAt: currentScroll });
        }
      }
  ...
  const makeStatic = async (item: any) => {
    ...
    try {
      // Clear static from others
      const currentStatics = urgentItems.filter(i => i.isActive !== false && i.id !== item.id);
      for (const other of currentStatics) {
        const scrollEnd = other.scrollingExpiresAt || other.expiresAt || (Date.now() + 1440 * 60000);
        await updateDoc(doc(db, "urgentNews", other.id), { staticExpiresAt: 0, scrollingExpiresAt: scrollEnd });
      }
      
      const newStaticExp = Date.now() + duration * 60000;
      const currentScroll = item.scrollingExpiresAt || item.expiresAt || Date.now();
      await updateDoc(doc(db, "urgentNews", item.id), { 
        staticExpiresAt: newStaticExp,
        scrollingExpiresAt: Math.max(currentScroll, newStaticExp)
      });
  ...
  const editItem = (item: any) => {
    ...
    const staticLeft = Math.max(0, Math.ceil((item.staticExpiresAt - Date.now()) / 60000));
    setStaticMinutes(staticLeft);
    
    const scrollEnd = item.scrollingExpiresAt || item.expiresAt || Date.now();
    let scrollLeft = Math.max(0, Math.ceil((scrollEnd - Date.now()) / 60000));
    if (staticLeft > 0) scrollLeft = Math.max(0, scrollLeft - staticLeft);
    setScrollingMinutes(scrollLeft);
  ...
  const expiredDocs = normalizedItems.filter(i => i.staticExpiresAt <= currentTime && i.scrollingExpiresAt <= currentTime && i.isActive !== false);
  const cancelledDocs = normalizedItems.filter(i => i.isActive === false);
  ...
  const stats = [
    ...
    { label: "الأرشيف (كافة الأخبار)", value: normalizedItems.length, color: "text-slate-600", bg: "bg-slate-50 dark:bg-slate-900/50", border: "border-slate-200 dark:border-slate-800", icon: <Archive className="w-6 h-6" /> }
  ];
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-6xl mx-auto font-cairo">
      ...
      <div className="bg-gray-100/50 dark:bg-gray-900/50 p-1.5 rounded-2xl flex flex-wrap gap-1.5 border border-gray-200 dark:border-gray-800">
        ...
        <button 
          onClick={() => { setActiveTab("add"); setEditingId(null); setText(""); setStaticMinutes(60); setScrollingMinutes(1440); }}
                  ...
                  return (
                    <div key={item.id} className={\`bg-white dark:bg-gray-800 p-5 rounded-2xl border \${isStatic ? 'border-red-300 dark:border-red-700 shadow-md ring-2 ring-red-50 dark:ring-red-900/20' : 'border-gray-200 dark:border-gray-700 shadow-sm'} transition-all\`}>
                      <div className="flex flex-col md:flex-row gap-5 items-start">
                        ...
                        {/* Actions */}
                        <div className="flex flex-col gap-2 shrink-0 w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-gray-100 dark:border-gray-700">
                          <div className="flex flex-row gap-2">
                            {isStatic ? (
                              <button onClick={() => moveToMarquee(item)} className="flex-1 justify-center bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-1.5 transition-colors">
                                <ArrowLeftRight className="w-4 h-4" /> نقل للمتحرك فوراً
                              </button>
                            ) : (
                              <button onClick={() => makeStatic(item)} className="flex-1 justify-center bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:hover:bg-red-900/50 text-red-700 dark:text-red-300 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-1.5 transition-colors">
                                <AlertTriangle className="w-4 h-4" /> جعله ثابتاً
                              </button>
                            )}
                            {!isScrolling && (
                              <button onClick={() => makeScrolling(item)} className="flex-1 justify-center bg-amber-50 hover:bg-amber-100 dark:bg-amber-900/30 dark:hover:bg-amber-900/50 text-amber-700 dark:text-amber-300 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-1.5 transition-colors">
                                <Activity className="w-4 h-4" /> جعله متحركاً
                              </button>
                            )}
                          </div>
                          <div className="flex flex-row gap-2">
                            <button onClick={() => editItem(item)} className="flex-1 justify-center bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-1.5 transition-colors">
                              <Edit2 className="w-4 h-4" /> تعديل
                            </button>
                            <button onClick={() => toggleCancelItem(item.id, true)} className="flex-1 justify-center bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:hover:bg-red-900/50 text-red-700 dark:text-red-300 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-1.5 transition-colors">
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm space-y-6">
              <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-700 pb-4">
                ...
                {editingId && (
                  <button onClick={() => { setEditingId(null); setText(""); setStaticMinutes(60); setScrollingMinutes(1440); }} className="text-gray-500 hover:text-red-600 text-sm font-bold flex items-center gap-1 bg-gray-50 px-3 py-1.5 rounded-lg">
              ...
              <div className="space-y-5">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 flex items-center gap-2">
                    <Clock className="w-4 h-4 text-gray-500" /> مدة البقاء كخبر ثابت (بالدقائق):
                  </label>
                  <input 
                    type="number"
                    ...
                    min={0}
                  />
                  <p className="text-[10px] text-gray-500 mt-1">صفر (0) لعدم إظهاره كخبر ثابت ونقله للمتحرك مباشرة.</p>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2 flex items-center gap-2">
                    <Clock className="w-4 h-4 text-gray-500" /> مدة البقاء في الشريط المتحرك (بالدقائق):
                  </label>
                  <input 
                    type="number"
                    className="w-full p-4 text-lg font-bold bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-inner"
                    value={scrollingMinutes}
                    onChange={(e) => setScrollingMinutes(Number(e.target.value))}
                    min={0}
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 font-bold leading-relaxed">
                    بعد انتهاء المدتين (ثابت ثم متحرك) سيتم إيقاف عرض الخبر تلقائياً وتوثيقه في الأرشيف.
                  </p>
                </div>
                    ...
                    <div key={item.id} className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col transition-all hover:shadow-md">
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          {isCancelled ? (
                            ...
                            <span className="text-[10px] font-black text-red-600 bg-red-50 dark:bg-red-950 px-2 py-1 rounded-md border border-red-200 flex items-center gap-1">
                              <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span> خبر ثابت
                            </span>
                          ) : (
                            <span className="text-[10px] font-black text-amber-600 bg-amber-50 dark:bg-amber-950 px-2 py-1 rounded-md border border-amber-200 flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-2 pt-3 border-t border-gray-100 dark:border-gray-700 mt-auto">
                        <button onClick={() => republishItem(item)} className="flex-1 bg-gray-50 hover:bg-gray-100 dark:bg-gray-900 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs font-bold px-3 py-2 rounded-xl flex items-center justify-center gap-1.5 transition-colors">
                          <RotateCcw className="w-4 h-4" /> تعديل وإعادة نشر
                        </button>
