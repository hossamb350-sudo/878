...
const componentCode = `function AdminUrgentNews() {
  ...
  const [displayType, setDisplayType] = useState<"static" | "scrolling">("scrolling");
  const [durationMinutes, setDurationMinutes] = useState(60);
  const [scrollingAfterStaticMinutes, setScrollingAfterStaticMinutes] = useState(1440);
  ...
  const saveNews = async () => {
    ...
    if (durationMinutes <= 0) return alert("يرجى إدخال مدة صحيحة");
    ...
    try {
      ...
      if (displayType === "static") {
        newStaticExpiresAt = now + durationMinutes * 60000;
        newScrollingExpiresAt = newStaticExpiresAt + scrollingAfterStaticMinutes * 60000;
        
        // Remove static status from other news items (only one static allowed at a time)
        ...
        newScrollingExpiresAt = now + durationMinutes * 60000;
      }
      ...
      setScrollingAfterStaticMinutes(1440);
      setActiveTab("active");
    } catch (e) {
      console.error(e);
  ...
  const makeStatic = async (item: any) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "urgentNews", item.id), { 
        ...
        scrollingExpiresAt: Math.max(currentScroll, newStaticExp + 60 * 60000) // Default 1 hour scrolling after static
      });
  ...
  const editItem = (item: any) => {
    ...
    setDisplayType(isStatic ? "static" : "scrolling");
    
    if (isStatic) {
      const staticLeft = Math.max(0, Math.ceil((item.staticExpiresAt - now) / 60000));
      setDurationMinutes(staticLeft);
      
      const scrollEnd = item.scrollingExpiresAt || item.expiresAt || now;
      let scrollLeft = Math.max(0, Math.ceil((scrollEnd - item.staticExpiresAt) / 60000));
      setScrollingAfterStaticMinutes(scrollLeft);
    } else {
      const scrollEnd = item.scrollingExpiresAt || item.expiresAt || now;
      const scrollLeft = Math.max(0, Math.ceil((scrollEnd - now) / 60000));
      setDurationMinutes(scrollLeft);
    }
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-6xl mx-auto font-cairo">
      ...
      <div className="bg-gray-100/50 p-1.5 rounded-2xl flex flex-wrap gap-1.5 border border-gray-200">
        ...
        <button 
          onClick={() => { setActiveTab("add"); setEditingId(null); setText(""); setDisplayType("scrolling"); setDurationMinutes(1440); }}
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
              <div className="flex items-center justify-between border-b border-gray-100 pb-4">
                ...
                {editingId && (
                  <button onClick={() => { setEditingId(null); setText(""); setDisplayType("scrolling"); setDurationMinutes(1440); }} className="text-gray-500 hover:text-red-600 text-sm font-bold flex items-center gap-1 bg-gray-50 px-3 py-1.5 rounded-lg">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <textarea
                    className="w-full p-5 text-lg font-bold bg-gray-50 border border-gray-200 rounded-2xl h-40 focus:ring-2 focus:ring-red-500 transition-all resize-none shadow-inner"
                ...
                <div>
                  ...
                  <div className="flex gap-4">
                    <label className={\`flex-1 cursor-pointer p-4 rounded-xl border flex items-center gap-3 transition-all \${displayType === 'static' ? 'bg-red-50 border-red-300 ring-1 ring-red-200' : 'bg-gray-50 border-gray-200'}\`}>
                      <input 
                        type="radio" 
                        name="displayType"
                        checked={displayType === 'static'}
                        onChange={() => { setDisplayType('static'); setDurationMinutes(60); }}
                        className="w-5 h-5 text-red-600"
                      />
                      <div>
                        <div className="font-bold text-gray-900">خبر ثابت</div>
                        <div className="text-xs text-gray-500">يظهر فوق الشريط ويوقف أخباره</div>
                      </div>
                    </label>
                    <label className={\`flex-1 cursor-pointer p-4 rounded-xl border flex items-center gap-3 transition-all \${displayType === 'scrolling' ? 'bg-amber-50 border-amber-300 ring-1 ring-amber-200' : 'bg-gray-50 border-gray-200'}\`}>
                      <input 
                        type="radio" 
                        name="displayType"
                        checked={displayType === 'scrolling'}
                        onChange={() => { setDisplayType('scrolling'); setDurationMinutes(1440); }}
                        className="w-5 h-5 text-amber-600"
                      />
                      <div>
                        <div className="font-bold text-gray-900">خبر متحرك</div>
                        <div className="text-xs text-gray-500">ينضم تلقائياً للشريط المتحرك</div>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2 flex items-center gap-2">
                    ...
                    {displayType === 'static' ? 'مدة العرض كخبر ثابت (بالدقائق):' : 'مدة العرض في الشريط المتحرك (بالدقائق):'}
                  </label>
                  <input 
                    type="number"
                    className="w-full p-4 text-lg font-bold bg-gray-50 border border-gray-200 rounded-2xl shadow-inner"
                  ...
                  {displayType === 'static' && (
                    <p className="text-xs text-gray-500 mt-2 font-bold leading-relaxed">
                      بعد انتهاء المدة كخبر ثابت، سيتم نقله للشريط المتحرك تلقائياً.
                    </p>
                  )}
                </div>
                ...
                {displayType === 'static' && (
                  <div>
                    ...
                    <input 
                      type="number"
                      className="w-full p-4 text-lg font-bold bg-gray-50 border border-gray-200 rounded-2xl shadow-inner"
                      value={scrollingAfterStaticMinutes}
                      onChange={(e) => setScrollingAfterStaticMinutes(Number(e.target.value))}
                      min={0}
                    />
                  </div>
                )}
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <input 
                    ...
                    max={120}
                  />
                  <p className="text-xs text-gray-500 mt-2 font-bold">السرعة الافتراضية 25، وتتكيف تلقائياً مع طول النصوص.</p>
                </div>
                  ...
                  return (
                    <div key={item.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col transition-all hover:shadow-md">
                      ...
                      <div className="flex items-center gap-2 pt-3 border-t border-gray-100 mt-auto">
                        <button onClick={() => republishItem(item)} className="flex-1 bg-gray-50 hover:bg-gray-100 text-gray-700 text-xs font-bold px-3 py-2 rounded-xl flex items-center justify-center gap-1.5 transition-colors">
