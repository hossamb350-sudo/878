...
new_func = """function AdminActivitiesContent() {
  ...
  const save = async () => {
    if (!actType || !actDate) return alert("يرجى إدخال نوع الفعالية والتاريخ");
    ...
    try {
      ...
      const data = {
        title: actType, // Fallback for backwards compatibility
        type: actType,
        dayName: actDayName,
  ...
  const uniqueTypes = Array.from(new Set(activities.map(a => a.type || a.title).filter(Boolean)));
        ...
        {activities.map((act) => (
          <div key={act.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-start md:items-center shadow-sm gap-4">
            <div className="flex-1">
              <h4 className="font-bold text-gray-900 dark:text-white text-lg">{act.type || act.title}</h4>
