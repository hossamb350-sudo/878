...
const replacement = `                      {/* Left Side News Content */}
                      <div className="flex-1 min-w-0 flex flex-col justify-between py-2 px-3 text-right">
                         <h3 className="font-bold text-[11px] sm:text-[12px] text-gray-900 leading-[1.5] transition-colors group-hover:text-taiz-sky mb-1 whitespace-normal line-clamp-3 font-cairo" style={{ fontFamily: 'Cairo, Tajawal, "IBM Plex Sans Arabic", sans-serif' }}>
                           {item.title}
                         </h3>

                         {/* Consistently aligned metadata line */}
                         <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1.5 mt-auto w-full pt-1">
                            {/* 1. Author */}
                            {item.author && (
                              <div className="flex items-center gap-1 text-slate-700 shrink-0 bg-slate-50/80 px-1.5 py-0.5 rounded border border-slate-100">
                                <PenTool className="w-2.5 h-2.5 text-taiz-sky" />
                                <span className="font-bold text-[9px] sm:text-[10px] truncate max-w-[85px]">{item.author}</span>
                              </div>
                            )}
                            ...
                            {/* 5. Views */}
                            <div className="flex items-center gap-1 text-slate-600 shrink-0 mr-auto bg-slate-50/80 px-1.5 py-0.5 rounded border border-slate-100">
                              <Eye className="w-3 h-3 text-taiz-sky"/> 
                              <span className="font-bold text-[9px] sm:text-[10px]">{item.views || 0}</span>
                              <span className="text-[8px] sm:text-[9px] hidden sm:inline">مشاهدات</span>
                            </div>
                         </div>
                      </div>`;
