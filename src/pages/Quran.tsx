...
export function Quran() {
  ...
  const handleSaveNote = async (paragraphIndex: number, noteText: string) => {
    if (!selectedLesson || !selectedSeries) return;
    const exNote = notes.find(n => n.lessonId === selectedLesson.id && n.paragraphIndex === paragraphIndex);
    
    const updated = exNote
      ? notes.map(n => (n.lessonId === selectedLesson.id && n.paragraphIndex === paragraphIndex) ? { ...n, noteText, updatedAt: Date.now() } : n)
      : [...notes, {
          lessonId: selectedLesson.id,
          seriesId: selectedSeries.id,
          lessonTitle: selectedLesson.title,
  ...
  const handleDeleteNote = async (paragraphIndex: number) => {
    if (!selectedLesson) return;
    const updated = notes.filter(n => !(n.lessonId === selectedLesson.id && n.paragraphIndex === paragraphIndex));
    setNotes(updated);
    localStorage.setItem("quran_notes", JSON.stringify(updated));
  ...
  const handleToggleHighlight = async (paragraphIndex: number, color: string) => {
    if (!selectedLesson) return;
    const exIdx = highlights.findIndex(h => h.lessonId === selectedLesson.id && h.paragraphIndex === paragraphIndex);
    ...
    if (isSameColor) {
      ...
      const item = { lessonId: selectedLesson.id, paragraphIndex, color, createdAt: Date.now() };
      if (exIdx > -1) {
    ...
    if (auth.currentUser) {
      try {
        ...
        if (isSameColor) {
          ...
          await setDoc(ref, { lessonId: selectedLesson.id, paragraphIndex, color, createdAt: Date.now() });
        }
      } catch(e){}
    }
  };
  ...
  const handleHopToLesson = (lessonId: string, paragraphIndex?: number) => {
  ...
  const ProgressBar = ({ percentage }: { percentage: number }) => (
    <div className="w-full bg-gray-200 dark:bg-gray-700 h-1 rounded-full overflow-hidden mt-2">
      <motion.div 
        initial={{ width: 0 }}
  ...
  const QuranSearchView = () => {
    const results = [
      ...lessonsList.filter(l => l.title.includes(searchQuery) || l.content.includes(searchQuery)).map(l => ({ ...l, type: 'lesson' as const })),
      ...excerptsList.filter(e => e.title.includes(searchQuery) || e.content.includes(searchQuery)).map(e => ({ ...e, type: 'excerpt' as const }))
    ];
            ...
            results.map(item => {
              ...
              return (
                <button 
                   ...
                   onClick={() => {
                     if (isLesson) {
                       const series = seriesList.find(s => s.id === item.seriesId);
                       if (series) navigateToLesson(item as QuranLesson, series);
                     } else {
                       setSelectedExcerpt(item as QuranExcerpt);
  ...
  const Header = () => {
    ...
    if (activeView === 'syllabuses') title = "مقررات الدروس";
    ...
    if (activeView === 'stats') title = "لوحة المتابعة وتحصيلي";
      ...
      <div className="space-y-4 max-w-lg mx-auto">
         {seriesList.length === 0 ? (
           ...
           seriesList.map(series => (
             <button 
                key={series.id} 
                onClick={() => { setSelectedSeries(series); setActiveView('lessons'); }}
  ...
  const LessonsView = () => {
    const seriesLessons = lessonsList.filter(l => l.seriesId === selectedSeries?.id);
    return (
      <div className="flex-1 overflow-y-auto px-4 py-8 relative" ref={scrollRef}>
        <div className="space-y-3 max-w-lg mx-auto">
           {seriesLessons.length === 0 ? (
             ...
             seriesLessons.map(lesson => {
                ...
                return (
                  <button 
                     key={lesson.id} 
                     onClick={() => navigateToLesson(lesson, selectedSeries!)}
    ...
    <div className="flex-1 overflow-y-auto px-4 py-8 relative" ref={scrollRef}>
      <div className="grid gap-4 max-w-2xl mx-auto md:grid-cols-2">
         {syllabusesList.length === 0 ? (
           ...
           syllabusesList.map(item => (
             <button 
                key={item.id} 
                onClick={() => { setSelectedSyllabus(item); setActiveView('syllabus-detail'); }}
      ...
      <div className="space-y-4 max-w-lg mx-auto">
         {excerptsList.length === 0 ? (
           ...
           excerptsList.map(item => (
             <button 
                key={item.id} 
                onClick={() => { setSelectedExcerpt(item); setActiveView('excerpt-detail'); }}
  ...
  const LessonDetailView = () => {
    ...
    return (
      <QuranReader 
        ...
        onClearJump={() => setJumpToParagraphIndex(null)}
      />
    );
  };
  ...
  return (
    <div className="flex-1 w-full flex flex-col bg-gray-50 dark:bg-gray-950 font-sans rtl relative overflow-hidden" dir="rtl">
      {activeView !== 'lesson-detail' && <Header />}
      <Sidebar />
