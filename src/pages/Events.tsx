...
export function Events() {
  ...
  const getEventStatus = (timestamp: number) => {
    ...
    if (isSameDay(eventDate, today)) return { label: "اليوم", color: "bg-taiz-sky", text: "text-taiz-sky border-taiz-sky/20 bg-taiz-sky/5" };
    if (isAfter(eventDate, today)) return { label: "قادمة", color: "bg-taiz-royal", text: "text-taiz-royal border-taiz-royal/20 bg-taiz-royal/5" };
    return { label: "منتهية", color: "bg-taiz-soft", text: "text-taiz-soft border-taiz-soft/20 bg-taiz-soft/5" };
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3 bg-surface-main">
        ...
        <p className="text-text-secondary text-xs font-bold">جاري تحميل تقويم المناسبات...</p>
      </div>
    );
  }
      ...
      <div className="relative overflow-hidden bg-surface-card rounded-3xl p-6 sm:p-8 border border-border-light shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="space-y-3 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2.5xl sm:text-3.5xl font-black text-text-primary mt-1">تقويم المناسبات</h1>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4 bg-surface-main px-5 py-3 rounded-2xl border border-border-light shadow-sm shrink-0 self-start md:self-center">
          <div className="text-right px-3">
             <div className="text-2xl font-black text-taiz-royal">{events.length}</div>
             <div className="text-[10px] font-black text-text-muted uppercase">إجمالي المناسبات</div>
          </div>
          <div className="w-px h-8 bg-border-light"></div>
          <div className="text-right px-3">
             <div className="text-2xl font-black text-taiz-sky">{upcomingEvents.length}</div>
             <div className="text-[10px] font-black text-text-muted uppercase">مناسبات قادمة</div>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-7 relative overflow-hidden bg-gradient-to-br from-taiz-navy to-taiz-royal rounded-[2rem] p-6 sm:p-8 text-white shadow-xl flex items-center min-h-[240px] group">
           ...
           <div className="relative z-10 w-full flex flex-col justify-between h-full space-y-6">
              <div className="flex items-center justify-between">
                ...
                <div className="text-taiz-soft font-black text-xs">{format(today, "EEEE, d MMMM yyyy", { locale: ar })}</div>
              </div>
              
              {eventOfTheDay ? (
                <div className="space-y-4 font-sans text-right">
                  <h2 className="text-2.5xl sm:text-3xl font-black leading-tight text-white group-hover:text-taiz-sky transition-colors duration-300">{eventOfTheDay.title}</h2>
                ...
                <div className="py-6 text-center w-full font-sans">
                   <div className="text-base font-black text-taiz-sky flex items-center justify-center gap-2 mb-1.5">
                      <Info className="w-5 h-5 text-white" /> لا توجد مناسبات مسجلة لهذا اليوم التاريخي
                   </div>
                   <p className="text-white/60 text-xs font-bold">تصفح التقويم وأقرب المناسبات من اللوحة المجاورة</p>
                </div>
              )}
           </div>
        </div>
        ...
        <div className="lg:col-span-5 relative overflow-hidden bg-surface-card rounded-[2rem] p-6 sm:p-8 border border-border-light shadow-sm flex flex-col justify-between group">
           ...
           {nearestUpcoming ? (
             <div className="relative z-10 flex flex-col h-full justify-between space-y-4">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border-light">
                   <div className="flex flex-col">
                      <span className="text-[10px] font-black text-text-muted">التاريخ المجدول</span>
                      <span className="text-xs font-black text-text-primary">{nearestUpcoming.hijriDate}</span>
                   </div>
                   <button 
             ...
             <div className="flex flex-col items-center justify-center h-full text-center text-text-muted py-10 opacity-60">
                ...
                <div className="font-black text-sm">لا توجد مناسبات قادمة مجدولة في المدى القريب</div>
             </div>
           )}
        </div>
      </div>
                 ...
                 <table className="w-full text-right border-collapse">
                    ...
                    <tbody className="divide-y divide-border-light">
                       {filteredEvents.map((event, idx) => {
                          ...
                          return (
                            <tr key={event.id} className="hover:bg-surface-hover transition-colors border-border-light">
                               <td className="p-4.5 text-xs font-black text-text-muted">{idx + 1}</td>
                               <td className="p-4.5">
                                  <div className="font-black text-text-primary text-sm">{event.title}</div>
                                  <div className="text-[10px] text-text-secondary font-black mt-0.5">{event.dayName}</div>
                               </td>
                               <td className="p-4.5 text-xs font-black text-taiz-royal">{event.hijriDate}</td>
                               <td className="p-4.5 text-xs font-bold text-text-secondary">{event.gregorianDate}</td>
                               <td className="p-4.5">
              ...
              <div className="flex flex-col items-center">
                 <div className="w-full max-w-4xl bg-surface-card rounded-[2rem] border border-border-light shadow-sm p-4 sm:p-8">
                    ...
                    <Calendar 
                       ...
                       tileContent={({ date }) => {
                          const hasEvent = events.some(e => isSameDay(new Date(e.timestamp), date));
                          return hasEvent ? <div className="event-dot bg-taiz-royal"></div> : null;
                       }}
                       onClickDay={(date) => {
                 ...
                 <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-4xl">
                    {events.filter(e => isAfter(new Date(e.timestamp), today) || isSameDay(new Date(e.timestamp), today)).slice(0, 4).map(e => (
                       <div key={e.id} onClick={() => setSelectedEventId(e.id)} className="flex items-center gap-4 bg-surface-card p-4.5 rounded-2xl border border-border-light shadow-sm cursor-pointer hover:border-taiz-sky/30 transition-all select-none">
                          ...
                          <div className="flex-1 text-right">
                             <h4 className="text-sm font-black text-text-primary truncate">{e.title}</h4>
                             <p className="text-[10px] font-black text-text-secondary">{e.hijriDate} • {e.gregorianDate}</p>
                          </div>
  ...
  return (
    ...
    >
       ...
       <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
          <span className="text-[10px] sm:text-xs font-black text-taiz-royal group-hover:underline flex items-center gap-1">عرض التفاصيل <ChevronLeft className="w-3.5 h-3.5" /></span>
          {remaining > 0 && (
    ...
    <div className={`relative flex items-center gap-6 ${isEven ? 'flex-row' : 'flex-row-reverse'}`}>
       ...
       <div className={`w-1/2 ${isEven ? 'text-left pl-2' : 'text-right pr-2'}`}>
          ...
          >
             ...
             <h4 className="text-xs sm:text-sm font-black text-taiz-navy mb-1 leading-tight">{event.title}</h4>
             <div className="text-[10px] text-taiz-soft font-bold">{event.gregorianDate}</div>
          </motion.div>
       </div>
...
function NoResultsFound({ className = "" }: { className?: string }) {
  return (
    <div className={`flex flex-col items-center justify-center py-20 bg-surface-main rounded-2xl border border-dashed border-border-light w-full ${className}`}>
       <Search className="w-10 h-10 text-text-muted mb-3" />
       <div className="text-sm font-black text-text-primary">مرحباً.. لا توجد نتائج مطابقة لبحثك</div>
       <p className="text-text-secondary text-xs mt-1 font-bold">تأكد من كتابة الكلمات بشكل صحيح وتصفية الفئات والمناسبات الفرعية</p>
    </div>
  );
}
...
function EventDetailsModal({ event, events, onClose }: { event: EventItem | undefined, events: EventItem[], onClose: () => void }) {
  ...
  const statusTheme = diff === 0 
    ? { btn: "bg-taiz-sky text-white", badge: "bg-taiz-sky/5 text-taiz-sky border-taiz-sky/10" } 
    : (diff > 0 
        ? { btn: "bg-taiz-royal text-white", badge: "bg-taiz-royal/5 text-taiz-royal border-taiz-royal/10" } 
        : { btn: "bg-taiz-navy text-white", badge: "bg-taiz-navy/5 text-text-primary border-border-light" });
       ...
       >
          ...
          <div className="flex items-start justify-between pb-4 border-b border-border-light">
             <div className="space-y-1 text-right">
                ...
                <h2 className="text-lg sm:text-xl font-black text-text-primary leading-tight mt-1">{event.title}</h2>
                <p className="text-xs text-text-secondary font-bold">{event.gregorianDate} - {event.dayName}</p>
             </div>
          ...
          <div className="grow overflow-y-auto py-5 space-y-5 text-right">
             ...
             <div className="space-y-2">
                <div className="flex items-center gap-1 px-1">
                   ...
                   <span className="text-xs font-black text-text-secondary">ملخص وموضوع المناسبة</span>
                </div>
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className="bg-taiz-sky/5 border border-taiz-sky/10 p-4 rounded-xl">
                   <span className="text-[10px] font-black text-taiz-royal block mb-0.5">التاريخ الهجري</span>
                   <span className="text-xs sm:text-sm font-black text-text-primary">{event.hijriDate}</span>
                </div>
                <div className="bg-taiz-royal/5 border border-taiz-royal/10 p-4 rounded-xl">
                   <span className="text-[10px] font-black text-text-primary block mb-0.5">تباعد الأيام للحدث</span>
             ...
             <div className="space-y-2">
                <div className="flex items-center gap-1 px-1">
                   ...
                   <span className="text-xs font-black text-text-secondary">الربط والمسافات الزمنية</span>
                </div>
                <div className="grid grid-cols-1 gap-2 text-xs font-black">
                   {prevEvent && (
                     <div className="flex items-center justify-between p-3.5 bg-surface-main hover:bg-surface-hover rounded-xl transition border border-border-light">
                        <span className="text-text-secondary leading-none">فترة البعد عن [{prevEvent.title.slice(0, 20)}...]:</span>
                        <span className="text-taiz-royal font-black">+{daysFromPrev} يوم</span>
                     </div>
                   )}
                   {nextEvent && (
                     <div className="flex items-center justify-between p-3.5 bg-surface-main hover:bg-surface-hover rounded-xl transition border border-border-light">
                        <span className="text-text-secondary leading-none">فترة القرب من [{nextEvent.title.slice(0, 20)}...]:</span>
                        <span className="text-taiz-sky font-black">{daysToNext} يوم</span>
                     </div>
                   )}
                </div>
             </div>
