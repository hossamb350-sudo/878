...
import { 
  ...
  ChevronRight, ChevronLeft, Search, Users, ExternalLink, Sliders, LayoutGrid, Loader2, Edit2, Zap, Activity 
} from "lucide-react";
...
export function AdminNewsWizard({ isAdmin, onBackToDashboard }: NewsWizardProps) {
  ...
  // Quick Inline Category Update for News List
  const handleQuickUpdateCategory = async (id: string, newCategory: string, allCategories: string[]) => {
    try {
      ...
      setNewsList(prev => {
        const updated = prev.map(n => n.id === id ? { ...n, ...updateData } : n);
        SyncService.setCache("news", updated).catch(() => {});
        return updated;
      });
    } catch (error) {
