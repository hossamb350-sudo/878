...
const Interactive3DHijriIllustration = () => (
  <motion.div 
    ...
    whileTap={{ scale: 0.96 }}
    className="relative w-24 h-24 xs:w-28 xs:h-28 sm:w-36 sm:h-36 flex items-center justify-center shrink-0 drop-shadow-2xl select-none cursor-pointer transition-transform"
  >
    <img 
...
export function HijriCalendarWidget({ onSelectEvent }: HijriCalendarWidgetProps) {
  ...
  const [hijriMonth, setHijriMonth] = useState<number>(3); // default Rabi' al-Awwal
  const [hijriYear, setHijriYear] = useState<number>(1448);
  
  // Selected Hijri day number
  const [selectedDayNumber, setSelectedDayNumber] = useState<number>(17);
  ...
  // Single Source of Truth dataset containing strictly events / occasions
  ...
  // Initialize with current real Hijri date
  useEffect(() => {
    ...
    try {
      ...
      setTodayHijri({ day: 17, month: 3, year: 1448 });
    }
  }, []);
  ...
  return (
    <div className="w-full max-w-md md:max-w-xl mx-auto space-y-3.5 sm:space-y-4 select-none font-sans" dir="rtl">

      {/* 1. MAIN HERO ISLAMIC CARD */}
      <motion.div 
      ...
      >
        ...
        <div className="relative z-10 flex items-start justify-between pt-3 sm:pt-4 pb-2">
      ...
      {/* 2. START & END OF MONTH SIDE-BY-SIDE CARDS */}
      ...
      >
        ...
        {/* Occasion Details for Selected Day (if any) */}
      ...
      {/* 4. UPCOMING EVENTS IN THIS MONTH SECTION */}
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
            ...
            {currentMonthEvents.map((ev, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  onClick={() => {
                    setSelectedDayNumber(ev.primaryDay);
